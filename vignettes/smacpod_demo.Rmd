...
The function relies internally on `spatstat.explore::Kest`, and users are directed there for more details about the estimation process. The `kest` function produces on object of class `fv`, defined by the **spatstat** package, which has an associated `plot` method that can be used to plot the results. 
