...
logrr = function(x, sigma = NULL, sigmacon = NULL, case = 2, 
  ...
  if (nsim > 0) {
    # compute log relative risk for relabeled data sets nsim times
    simr2 <- pbapply::pblapply(seq_len(nsim), function(i) {
      # randomly label cases
      cases = sample(x$n, N1)
      # compute density for newly relabeled cases
      fsim = spdensity(x = x[cases,], sigma = sigma, ..., 
                       ...
                       scalekernel = scalekernel,
                       positive = positive, verbose = verbose)
      # compute density for newly relabeled controls
      gsim = spdensity(x = x[-cases,], sigma = sigmacon, ..., 
                       ...
                       scalekernel = scalekernel,
                       positive = positive, verbose = verbose)
      # return the log relative risk of the relabeled data
      log(fsim$v) - log(gsim$v)
    })
    # move first simulated log relative risk to last + 1 position
    simr2[[nsim + 1]] <- simr2[[1]]
    # replace first simulated log relative risk with observed log
    # relative risk (for identifying and plotting purposes)
    simr2[[1]] <- r$v
    # stack elements of list into array
    simr2 <- abind::abind(simr2, along = 3)
    # save results for the later
    r$simr = simr2
    # compute envelopes for r based on level and alternative
    r$nrenv = nrenv(r, level = level, alternative = alternative)
    class(r) = c("logrrenv", class(r))
  }
  # include ppp window in r
  r$window = x$window
  # return object
  return(r)
}
